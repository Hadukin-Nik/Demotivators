...
import demotivators.helper_s.HelperForDAO_s;
...
public class UsersDAO {
    ...
    public static boolean addUser(User user, String login, String password) {
        try (Connection conn = DriverManager.getConnection(
            ...
            int numberOfUsers = HelperForDAO_s.howManyUsers("users");
            ...
            st.executeUpdate("INSERT INTO public.users(" +
                    ...
                    login+"\', \'"+ HelperForDAO_s.hashString(password)+"\', \'"+numberOfUsers+"\', \'" +user.getAvatar()+ "\');");
    ...
    public static User checkUser(String login) {
        try (Connection conn = DriverManager.getConnection(
                "jdbc:postgresql://127.0.0.1:5432/postgres", "postgres", "postgres")) {
            if(HelperForDAO_s.howManyUsers("users") <= 0) return null;
    ...
    public static User checkUser(String login, String password) {
        try (Connection conn = DriverManager.getConnection(
            ...
            String execute = "SELECT * FROM users WHERE \"Login\" = \'" + login + "\' " + " AND \"Password\" = \'" + HelperForDAO_s.hashString(password) + "\';";
